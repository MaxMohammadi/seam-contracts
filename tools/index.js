...
async function init(output) {
  ...
  app.get("/watch/:mtime", async (req, res) => {
    ...
    const fileMtime = fs.statSync("SeamBank.sol").mtime.getTime();
    ...
    var watcher = fs.watch("SeamBank.sol", "utf-8", (event, filename) => {
      ...
      compile().then(
        (output) =>
          res.json({
            changed: true,
            output,
            mtime: fs.statSync("SeamBank.sol").mtime.getTime(),
          }),
        (error) => console.error(error)
      );
    });
